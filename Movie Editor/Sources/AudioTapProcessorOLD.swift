...
final class AudioTapProcessorOLD {
    ...
    func process(buffer: UnsafeMutablePointer<AudioBufferList>) {
        ...
        let spectrum = makeLogSpectrum(magnitudesPerChannel: magnitudes, bandCount: self.spectrumBands, minFrequency: 20.0, maxFrequency: maxFreq, useMax: true)
    ...
    func makeLogSpectrum(
        ...
        func makeLogSpectrum(
            ...
            if useMax {
                // Take maximum magnitude across all channels and bins in each band
                ...
                // Average magnitudes across channels and bins
                for ch in 0..<channelCount {
                    let mags = magnitudesPerChannel[ch]
                    for bin in firstBin...lastBin {
                        let idx = map[bin]
                ...
                for i in 0..<bandCount {
                    ...
                    if count > 0 {
                        ...
                        // Band has no bins - use interpolation from neighbors
                        bands[i] = interpolateBand(bands: bands, index: i, bandCount: bandCount)
                    }
                }
            }
    ...
    /// Rebuild the bin-to-band mapping with proper logarithmic spacing
        private func rebuildBinToBandMapping(
            bandCount: Int,
            minFrequency: Float,
            maxFrequency: Float,
            spectrumSize: Int,
            ...
            // Map each bin to its corresponding log band
            for bin in firstBin...lastBin {
                ...
                let clamped = min(max(normalizedLog, 0.0), 0.9999)  // Prevent rounding to bandCount
                let bandIndex = Int(clamped * Float(bandCount))
                
                // Double-check bounds (should never trigger with clamped value)
                let safeBandIndex = min(max(bandIndex, 0), bandCount - 1)
                
                map[bin] = safeBandIndex
                binCounts[safeBandIndex] += 1
            }
            
            // Debug: Log any empty bands
            for (i, count) in binCounts.enumerated() where count == 0 {
                let bandMinFreq = pow(10.0, logMin + Float(i) * logRange / Float(bandCount))
                let bandMaxFreq = pow(10.0, logMin + Float(i + 1) * logRange / Float(bandCount))
                print("Warning: Band \(i) (\(bandMinFreq)-\(bandMaxFreq) Hz) has no bins")
            }
            ...
            print("""
            Log spectrum mapping created:
            - Bands: \(bandCount)
            - Frequency range: \(minFrequency)-\(maxFrequency) Hz
            - Bins mapped: \(binCounts.reduce(0, +)) / \(lastBin - firstBin + 1)
            - Bin counts per band: \(binCounts)
            """)
        }
        
        /// Interpolate value for empty bands using neighbors
        private func interpolateBand(bands: [Float], index: Int, bandCount: Int) -> Float {
...
/*
    func makeLogSpectrum(
        ...
        let firstBin = 1
        let lastBin  = spectrumSize - 2        // Nyquist included if <= maxFreq
